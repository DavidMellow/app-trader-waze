...
--app store columns	
	...
	10000 * a.price AS a_purch_price,--$10,000 * app price, but if app price <= $1, then $10,000 NEED TO ADD CASE STATMENT
	1000*(1+.5*a.rating)*12 AS a_proj_ls_maint_cost, --proj monthly maintenance cost is $1000 * lifespan in months
	primary_genre AS app_genre,
	a.content_rating AS app_content_rating,
--play store columns
	(((.5*5000)* ROUND(12*(p.rating/.5)+12,0))- (10000 * a.price) - (1000*(1+.5*p.rating)*12)) AS p_proj_net_profit,--projected life span rev.
	...
	10000 * a.price AS p_purch_price,--$10,000 * app price, but if app price <= $1, then $10,000 NEED TO ADD CASE STATMENT **NEED TO CHANGE a.price to p.price
